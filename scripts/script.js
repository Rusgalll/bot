...
let call = function(){ // функция всплывающего окна с вопросом и проверкой
    ...
    if(+question===number){ // проверка резульата когда угадал
       return alert('Поздравляю, Вы угадали!!! Хотели бы сыграть еще?'), attempt=0, number=getRandomIntInclusive(0,100), call(); // конец игры
        ...
        return alert('Попытки закончились, хотите сыграть еще ?'),attempt=0,number=getRandomIntInclusive(0,100), call(); // если попытки кончились
    }
}
};
